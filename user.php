...
$sql2 = "SELECT * FROM posts WHERE user_id=?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
      ...
      $stmt->execute();
      if ($stmt->error) { die('Execute failed: ' . $stmt->error); }
      $stmt->close();
  } else {
...
<body>
  ...
  <img src="log.jpg" alt="BlogPlatform logo">
    <h1><?php echo htmlspecialchars($username); ?> Posts!</h1>
    <nav>
        <ul>
  ...
  <div class="main" id="main">
    <div class="posts">
        <?php foreach ($posts as $post): ?>
          <div class="post-card">
            ...
            <button class="update-btn"><a href="updatepost.php?post_id=<?php echo $post['post_id']; ?>">Update</a></button>
            </div>
        </div>
        <?php endforeach; ?>
   ...
   <?php else: ?>
    ...
    <div class="container">
      ...
      <form id="postForm" action="user.php" method="post" enctype="multipart/form-data">
